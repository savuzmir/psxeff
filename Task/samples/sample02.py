...
win = psychopy.visual.Window(size=[1920, 1080], units='norm', monitor='default')
controller = tobii_controller(win)
controller.open_datafile('test.tsv', embed_events=True)
...
waitkey = True
while waitkey:
    controller.record_event('test')
    currentGazePosition = controller.get_current_pupil_size()
